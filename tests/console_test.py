...
class TestHBNBCommand(unittest.TestCase):
    ...
    def test_fs_create(self):
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            cons.onecmd('create City name="San Francisco" population=884961')
            mdl_id = cout.getvalue().strip()
            clear_stream(cout)
            self.assertIn('City.{}'.format(mdl_id), storage.all().keys())
            cons.onecmd('show City {}'.format(mdl_id))
            self.assertIn("'name': 'San Francisco'", cout.getvalue().strip())
            self.assertIn("'population': 884961", cout.getvalue().strip())
            clear_stream(cout)
            cons.onecmd('create User name="Alice" age=25 height=5.7')
            mdl_id = cout.getvalue().strip()
    ...
    def test_db_create(self):
        with patch('sys.stdout', new=StringIO()) as cout:
            cons = HBNBCommand()
            clear_stream(cout)
            cons.onecmd('create User email="john.doe@example.com" password="P@ssw0rd"')
            mdl_id = cout.getvalue().strip()
            dbc = MySQLdb.connect(
    ...
    def test_db_show(self):
        with patch('sys.stdout', new=StringIO()) as cout:
            ...
            obj = User(email="alice@example.com", password="Secret123")
            dbc = MySQLdb.connect(
                host=os.getenv('HBNB_MYSQL_HOST'),
                port=3306,
                user=os.getenv('HBNB_MYSQL_USER'),
            ...
            self.assertIn('alice@example.com', cout.getvalue())
            self.assertIn('Secret123', cout.getvalue())
            cursor.close()
            dbc.close()
